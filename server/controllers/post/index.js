...
module.exports = {
  getAllPost: async (req, res) => {
    ...
    try {
      let find = await sequelize.query(`
        SELECT id, title, image, content, lat, lng, address, categoryId, 
  ...
  getOnePost: async (req, res) => {
    ...
    */
    ...
    try {
      ...
      if (!find[0]) return res.status(400).json({message: 'Page Not Found'});
      ...
      if (!find.public) {
        if (!userData || find.userId !== userData.id) return res.status(400).json({message: 'Not authorized'})
      }
  ...
  uploadPost: async (req, res) => {
    ...
    if (!image) return res.status(400).json({message: '사진은 최소 1장 이상이 필요합니다'})
  ...
  patchPost: async (req, res) => {
    ...
    if (!userData.admin && find.userId !== userData.id) return res.status(400).json({message: 'Not authorized'})
  ...
  deletePost: async (req, res) => {
    ...
    if (!userData.admin && postData.userId !== userData.id)
  ...
  getPostImg: async (req, res) => {
    if (!req.body.lnk) return res.sendSattus(400)
  ...
  reportPost: async (req, res) => {
    if (!req.body.url) return res.sendSattus(400)
